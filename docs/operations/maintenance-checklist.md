...
- Also bump the target-key namespace after changing setup semantics that affect Cargo's environment, such as moving `MISE_DATA_DIR`, `MISE_RUSTUP_HOME`, cached target directories, switching from Rust/Zig installer actions to mise, or changing Cargo helper installation backends.
