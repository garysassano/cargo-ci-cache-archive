...
| D3 | Keep the source-keyed full-target cache (split Cargo home and target, target restored after `rust-cache`) as a proven workaround for affected local path workspace members that repeatedly rebuild on exact `rust-cache` hits. | Proven workaround | [Approach](../approaches/rust-cache-source-keyed-target-cache.md), [evidence](../evidence/cached-worktree-and-target-cache.md) |
...
- `Swatinem/rust-cache` with `cache-workspace-crates: true` and an mtime-preserving cached worktree can produce warm Cargo no-op builds. Its target caching is not equivalent to a full local target snapshot, so affected local path workspace members can still repeatedly rebuild when an exact target-cache hit restores stale state and the target key does not include workspace source contents. See [`rust-cache` behavior](../concepts/rust-cache-behavior.md).
